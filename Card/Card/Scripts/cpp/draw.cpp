...
void drawMyChara() {
	...
	while (i < MYCHARA_NUM) {
		...
		case ALIVE:
			int myCharaXpos = myChara[i].data.x - player.data.x, myCharaYpos = myChara[i].data.y - player.data.y;			//プレイヤーの座標とキャラの座標の差

			if (myChara[i].data.muki == 4 || myChara[i].data.muki == 5)myChara[i].data.img = myChara[i].data.WG[myChara[i].data.x % MAP_SIZE / 8 + 2 * 4];//マイキャラが斜め移動（右上、右下）のときの画像セット
			else if (myChara[i].data.muki == 6 || myChara[i].data.muki == 7)myChara[i].data.img = myChara[i].data.WG[myChara[i].data.x % MAP_SIZE / 8 + 1 * 4];//マイキャラが斜め移動（左上、左下）のときの画像セット
			else myChara[i].data.img = myChara[i].data.WG[(myChara[i].data.x % MAP_SIZE + myChara[i].data.y % MAP_SIZE) / 8 + myChara[i].data.muki * 4];//マイキャラの向きを考慮した十字移動の主人公の画像のセット。
			SetDrawBlendMode(DX_BLENDMODE_ALPHA, 80);//結構透明
...
void drawEnemy(int i) {
		...
		case ALIVE:
			int enemyXpos = enemyChara[i].data.x - player.data.x, enemyYpos = enemyChara[i].data.y - player.data.y;			//プレイヤーの座標とモンスターの座標の差
...
void objectCollisonDraw() {//プレイヤーがオブジェクトに接触した際に表示されるUI　（ステータスなどを表示）
	if (player.collisonFlag == TRUE) {
		...
		SetDrawBlendMode(DX_BLENDMODE_ALPHA, 50);//////////////////////////////////結構透明
		DrawBox(5, 300, 635, 475, GetColor(0, 0, 255), TRUE);
		SetDrawBlendMode(DX_BLENDMODE_ALPHA, 130);/////////////////////////////////やや透明
		//HPバーの表示
		...
		DrawFormatString(450, 355, Cr[1] = GetColor(255, 255, 255), "レア度  %d", collisonObject.status.rearity);//レア数値	
		DrawFormatString(450, 370, Cr[1] = GetColor(255, 255, 255), "職  %d", collisonObject.status.Class);//クラス
		DrawFormatString(450, 385, Cr[1] = GetColor(255, 255, 255), "視認範囲  %d", collisonObject.status.range);//視認範囲
		DrawFormatString(450, 400, Cr[1] = GetColor(255, 255, 255), "隠ぺい率  %d", collisonObject.status.hide);//隠蔽率
		DrawFormatString(450, 415, Cr[1] = GetColor(255, 255, 255), "移動マス  %d", collisonObject.status.MS);//最大移動マス
		DrawFormatString(450, 460, Cr[1] = GetColor(255, 255, 255), "コスト  %d", collisonObject.status.cost);//コスト

		SetFontSize(40);
		DrawFormatString(560, 400, Cr[1] = GetColor(255, 255, 255),  "%d", collisonObject.status.nowMS);//残りの移動
